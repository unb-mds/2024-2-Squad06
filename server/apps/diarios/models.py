...
class Diario(models.Model):
    ...
    contratacoes = models.ManyToManyField(
        Contratacao, related_name="contratacao", blank=True
    )
    ...
    def get_valores_salvos(cls):
        ...
        for diario in diarios:
            contratacao = diario.contratacao.all().values(
                "valor_mensal", "valor_anual", "vigencia", "fornecedor"
            )
            resultados.append(
                {
                    "date": diario.date,
