...
class Controladores:
    ...
    def buscar_diarios_maceio(querystring, published_since, published_until):
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Erro ao buscar diários: {response.status_code} {response.text}"
            )
    ...
    def associar_valores_a_contratos(self, texto):
        ...
        for i in range(0, len(valores), 2):
            if contrato_idx >= len(contratos):
                break
            contrato = (
                contratos[contrato_idx] if contrato_idx < len(contratos) else None
            )
    ...
    def extrair_info_contratos(texto):
        ...
        padrao_vigencia = re.compile(
            r"VIG[Ê|E]NCIA:?\s+(?:DE\s+|DA\s+ARP)?:?\s*(\d+)", re.IGNORECASE
        )
    ...
    def extrair_fornecedores(texto):
        padrao_fornecedor = re.compile(
            r"(?:fornecedor registrado: empresa|fornecedor registrado|Fornecedor|Empresa|Contratado):?\s+([^\n\r]+(?:[^\n\r]*))",
        ...
        fornecedores = defaultdict(
            lambda: {"nome": "", "cnpj": "", "data de assinatura": ""}
        )
        linhas = texto.split("\n")
        ...
        for i, linha in enumerate(linhas):
            ...
            if match_fornecedor:
                ...
                if any(
                    palavra.lower() in nome.lower() for palavra in palavras_ignoradas
                ):
                    continue
        ...
        for i in range(len(secoes) - 1):
            if re.search(r"(CONTRATO|CONTRATAÇÃO)", secoes[i], flags=re.IGNORECASE):
                if bloco_atual:
                    ...
                    if not re.search(
                        palavras_proibidas, bloco_completo, flags=re.IGNORECASE
                    ):
                        blocos_contratos.append(bloco_completo)
                bloco_atual = [secoes[i], secoes[i + 1]]
            elif re.search(
                r"(INSTITUTO DE|FUNDAÇÃO|PROCURADORIA)", secoes[i], flags=re.IGNORECASE
            ):
                if bloco_atual:
                    bloco_completo = "".join(bloco_atual)
                    if not re.search(
        ...
        for diario in diarios:
            try:
                caminho_arquivo = self.baixar_arquivo(
                    diario["txt_url"], f"{diario['date']}.txt"
                )
                with open(caminho_arquivo, "r", encoding="utf-8") as arquivo:
                    ...
                    if blocos_contratos:
                        for bloco in blocos_contratos:
                            ...
                            data_assinatura = (
                                contratos[0]["data_assinatura"] if contratos else "/"
                            )
                            if valores:
                                ...
                                for fornecedor_data in fornecedores.values():
                                    contratacao = {
                                        ...
                                        "valores": {"mensal": mensal, "anual": anual},
                                        "data_assinatura": data_assinatura,
                                        "vigencia": vigencia,
                                    }
    ...
    def carregar_dados_diarios(self):
        ...
        try:
            ...
            print(
                f"Carregamento concluído com sucesso: {len(resultados)} diários processados."
            )
        except Exception as e:
