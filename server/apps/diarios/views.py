...
class RequisicaoAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"erro": f"Datas inválidas: {ve}"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get(self, request):
        ...
        if data_assinatura:
            try:
                ...
                return JsonResponse(
                    {"error": "Data de assinatura inválida. Use o formato YYYY-MM-DD."},
                    status=400,
                )
        ...
        for diario in qs:
            diario_data = {
                "id": diario.id,
                "data_publicacao": (
                    diario.date.strftime("%Y-%m-%d") if diario.date else None
                ),
                "url": diario.url,
            ...
            for contratacao in diario.contratacoes.all():
                if len(contratacao.fornecedor.nome) > 6:
                    contrato_data = {
                        ...
                        "data_assinatura": (
                            contratacao.data_assinatura.strftime("%Y-%m-%d")
                            if contratacao.data_assinatura
                            else None
                        ),
    ...
    def get(self, request):
        fornecedores = (
            Fornecedor.objects.annotate(
                nome_length=Length("nome"), contract_count=Count("fornecedor")
            )
            .filter(nome_length__gt=5)
...
class FornecedorByNameAPIView(APIView):
    def post(self, request):
        ...
        if not fornecedor_nome:
            return Response(
                {"error": "Nome do fornecedor não fornecido."},
...
class DiariosPorFornecedorByIdAPIView(APIView):
    def post(self, request, id):
        ...
        if id == 0:
            ...
            if data_assinatura_str:
                try:
                    data_assinatura_date = datetime.strptime(
            ...
            if data_publicacao_str:
                try:
                    data_publicacao_date = datetime.strptime(
                        data_publicacao_str, "%Y-%m-%d"
                    ).date()
            ...
            for contratacao in diario.contratacoes.all():
                if len(contratacao.fornecedor.nome) > 6:
                    contrato_data = {
                        ...
                        "fornecedor": {
                            "id": (
                                contratacao.fornecedor.id
                                if contratacao.fornecedor
                                else None
                            ),
                            "nome": (
